package com.springer.patryk.korkidajmi.view.loggedin.mylessons.adapters
...
import com.springer.patryk.korkidajmi.view.loggedin.mylessons.MyLessonsContract
import kotlinx.android.synthetic.main.row_offer.view.*
import pl.mauto24.app.view.base.BaseRecyclerViewAdapter

class MyLessonsListAdapter(private val mPresenter: MyLessonsContract.Presenter) :
		BaseRecyclerViewAdapter<Offer, MyLessonsListAdapter.ViewHolder>() {
	...
	inner class ViewHolder(private val mItemView: View) : RecyclerView.ViewHolder(mItemView) {
		fun bind(offer: Offer) = with(mItemView) {
			(mItemView as CardView).setCardBackgroundColor(
					ContextCompat.getColor(context, offer.getColor()))
			...
			tv_row_lesson_address.text = offer.getAddress(mPresenter.getIsUserTutor())
